...
export default function GuestInterfaceClient({ roomId }: GuestInterfaceClientProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-8 relative" style={{ background: theme.backgroundColor }}>
      ...
      {/* Gönderilen İstekler Listesi */}
      <GonderilenIstekler roomId={roomId} onRequestDeleted={() => addNotification('success', safeGetTranslation('notifications.request_deleted', 'Talep Silindi'), safeGetTranslation('notifications.request_deleted_message', 'Talebiniz başarıyla silindi.'))} />

                </div>
  );
}

function GonderilenIstekler({ roomId, onRequestDeleted }: { roomId: string; onRequestDeleted: () => void }) {
  ...
  useEffect(() => {
    const loadRequests = async () => {
      try {
        ...
        const data = await ApiService.getGuestRequests(roomNumber);
        // Sadece bu odaya ait ve iptal edilmemiş istekleri göster
        const filteredRequests = data.filter((req: any) => 
          req.roomId === roomId && req.status !== 'cancelled'
        );
  ...
  const handleDeleteRequest = async (requestId: string) => {
    if (!confirm(safeGetTranslation('notifications.confirm_delete', 'Bu talebi silmek istediğinizden emin misiniz?'))) {
      return;
    }
    ...
    try {
      ...
      // Yine de listeden kaldır (optimistic update)
      setRequests(requests.filter(req => req.id !== requestId));
      onRequestDeleted();
    }
  };
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'completed':
        return safeGetTranslation('notifications.status_completed', 'Tamamlandı');
      case 'in_progress':
        return safeGetTranslation('notifications.status_in_progress', 'İşlemde');
      case 'pending':
        return safeGetTranslation('notifications.status_pending', 'Beklemede');
      default:
        return status;
    }
  };
  ...
  if (isLoading && requests.length === 0) {
    return null; // İlk yüklemede hiçbir şey gösterme
  }

  if (requests.length === 0) {
    return null; // İstek yoksa hiçbir şey gösterme
  }

  return (
    <div className="w-full max-w-md mx-4 mt-4">
      <div className="rounded-xl shadow p-4" style={{ background: theme.cardBackground }}>
        <h3 className="text-lg font-semibold mb-3" style={{ color: theme.textColor }}>
          {safeGetTranslation('room.sent_requests', 'Gönderilen İstekler')}
        </h3>
        <div className="space-y-2">
          {requests.map((request) => (
            <div
              ...
              style={{ background: `${theme.borderColor}20` }}
            >
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 text-xs" style={{ color: theme.textColor, opacity: 0.7 }}>
                  <span>{getStatusText(request.status)}</span>
                  <span>•</span>
                  <span>{new Date(request.createdAt).toLocaleString('tr-TR', { 
                    day: '2-digit', 
                    month: '2-digit', 
                    hour: '2-digit', 
                    minute: '2-digit' 
              ...
              {request.status === 'pending' && (
                <button
                  ...
                  className="ml-2 p-2 rounded-lg hover:bg-red-100 transition-colors"
                  style={{ color: '#ef4444' }}
                  title={safeGetTranslation('room.delete_request', 'Sil')}
                >
